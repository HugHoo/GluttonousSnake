...
static struct fixed {
	short fix[ROW][COLUMN];     //记录空间中每个坐标的状态，0为无，1为存在square
	int color[ROW][COLUMN];
};

struct fixed initial_fixed(struct fixed * fixed);                   //初始化fixed方块
inline void gotoxy(COORD pos);                                      //移动光标到pos位置
void print_square(struct square * form, const char ch[]);                   //打印方块
void print_fixed(struct fixed * fixed);                             //打印fixed的方块
void move_square(struct square * form, short xdir, short ydir);       //x轴移动xdir个单位，y轴移动ydir个单位
square create_square();                                               //创造方块
static square form1();                                               //7种类型的方块
static square form2();
static square form3();
static square form4();
static square form5();
static square form6();
static square form7();
square turn(square * pform);                                          //方块转动
bool turn_success(struct square * form, struct fixed * fixed);       //是否能够转动成功
static inline COORD tempXY();
bool is_buttom(struct square * form, struct fixed * fixed);                  //判断是否到达底部
bool is_wall(struct square * form, struct fixed * fixed, short dir);         //判断是否碰到墙壁/fixed的方块
struct fixed fix_square(struct square * form, struct fixed * fixed);          //fixed方块
...
void eliminate(struct fixed * fixed, struct square * form);                  //消行
...
square create_square()        //产生随机square类型
{
	srand(time(NULL));
	short random = rand() % 7;
	typedef square(*random_square)();
	random_square psquare[7] = {
		form1, form2, form3, form4, form5, form6, form7
	};
	return psquare[random]();
}

static square form1()        //类型1：L型
{
	COORD temp = tempXY();
	square form{
		1,
...
void eliminate(struct fixed * fixed, struct square * form)           //no problem
{
	short line[4];
	short t = 0;
	for (int i = 0; i < 3; i++)
